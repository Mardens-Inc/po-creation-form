...
const CONFIRM_EMAIL_TEMPLATE: &str = include_str!("../../templates/confirm_email_template.html.tera");
const RESET_PASSWORD_TEMPLATE: &str = include_str!("../../templates/reset_password_template.html.tera");
...
impl EmailService{
	pub fn new() -> Result<Self> {
		debug!("Creating SMTP transport with credentials: {}@{}", SMTP_USERNAME, SMTP_HOST);
		let creds = Credentials::new(SMTP_USERNAME.to_string(), SMTP_PASSWORD.to_string());
		let transport = SmtpTransport::starttls_relay(SMTP_HOST)?.credentials(creds).build();
		Ok(Self { transport })
	}

	pub async fn send_confirmation_email(&self, email_address: &str, token: &str, first_name: &str) -> Result<()> {
